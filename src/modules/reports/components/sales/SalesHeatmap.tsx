...
// Los JSON de geografia son TopoJSON: la topologia entera con sus "objects".
// topojson-client no trae tipos, asi que se describe lo minimo que se usa.
...
type TopoInput = Parameters<typeof topoFeature>[0];
type TopoObject = Parameters<typeof topoFeature>[1];

function topoFeatures(topo: Topology, objectName: string): GeoFeature[] {
  const collection = topoFeature(topo as unknown as TopoInput, topo.objects[objectName] as TopoObject);
...
const RANK_HEADERS: Record<'national' | 'provinces' | 'districts', string> = {
  national:  'Departamento',
  provinces: 'Provincia',
  districts: 'Distrito',
};
...
export function SalesHeatmap({ filters }: SalesHeatmapProps) {
  ...
  // Memorizado: los `?? []` creaban un array nuevo en cada render y hacian
  // recalcular los tres useMemo que dependen de el.
  const activeData = useMemo<SalesGeoHeatmapItem[]>(
    () => (view === 'districts' ? (provQuery.data ?? [])
  ...
  // Drill-down disparado desde la tabla de ranking: la fila ya trae el item,
  // asi que no hace falta resolver el geo_map del TopoJSON.
  const drillIntoItem = useCallback((item: SalesGeoHeatmapItem) => {
    if (view === 'national' && item.state_id && item.geo_map) {
      setSelectedDept({ geoMap: normGeo(item.geo_map), label: item.label, id: item.state_id });
      setHovered(null);
    } else if (view === 'provinces' && item.city_id && item.geo_map) {
      setSelectedProv({ geoMap: normGeo(item.geo_map), label: item.label, id: item.city_id });
      setHovered(null);
    }
  }, [view]);

  // Ranking del nivel activo: el backend ya ordena por total_revenue, pero la
  // metrica del selector puede ser "Pedidos".
  const ranking = useMemo(
    () => [...activeData].sort((a, b) => (b[metric] as number) - (a[metric] as number)),
    [activeData, metric],
  );

  const rankingTotal = useMemo(
    () => ranking.reduce((acc, item) => acc + (item[metric] as number), 0),
    [ranking, metric],
  );
    ...
    >
      {isLoading ? (
        ...
        <div className="grid gap-4 lg:grid-cols-[1fr_320px]">
          ...
          {/* Ranking del nivel activo — mismo dato que pinta el mapa */}
          <div className="h-[480px] overflow-hidden rounded-md border">
            <Table containerClassName="h-full">
              <TableHeader>
                <TableRow>
                  <TableHead className="h-9 px-3 text-xs">{RANK_HEADERS[view]}</TableHead>
                  <TableHead className="h-9 px-3 text-right text-xs">
                    {metric === 'total_revenue' ? 'Ingresos' : 'Pedidos'}
                  </TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {ranking.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={2} className="px-3 py-6 text-center text-xs text-muted-foreground">
                      Sin datos
                    </TableCell>
                  </TableRow>
                ) : (
                  ranking.map((item) => {
                    const canDrill = view === 'national' ? !!item.state_id
                                   : view === 'provinces' ? !!item.city_id
                                   : false;
                    return (
                      <TableRow
                        key={`${item.geo_map ?? 'sin'}-${item.label}`}
                        className={canDrill ? 'cursor-pointer' : undefined}
                        onClick={canDrill ? () => drillIntoItem(item) : undefined}
                        onMouseEnter={() => setHovered(item)}
                        onMouseLeave={() => setHovered(null)}
                      >
                        <TableCell className="px-3 py-2 text-xs">
                          <span className={canDrill ? 'font-medium' : undefined}>{item.label}</span>
                        </TableCell>
              ...
              {ranking.length > 0 && (
                <TableFooter>
                  <TableRow>
                    <TableCell className="px-3 py-2 text-xs">Total</TableCell>
                    <TableCell className="px-3 py-2 text-right text-xs tabular-nums">
                      {metric === 'total_revenue' ? formatCurrency(rankingTotal) : rankingTotal}
                    </TableCell>
                  </TableRow>
                </TableFooter>
              )}
