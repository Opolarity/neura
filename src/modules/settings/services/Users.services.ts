...
import { UsersApiResponse, UsersFilters, DocumentLookupPayload, DocumentLookupResponse } from "../types/Users.types";
...
export const getUserDocumentApi = async (userData: DocumentLookupPayload): Promise<DocumentLookupResponse> => {
...
export const getUsersFormDataApi = async (params?: { country_id?: number, state_id?: number, city_id?: number }) => {
    const queryParams = params ? "?" + new URLSearchParams(params as any).toString() : "";
    const { data, error } = await supabase.functions.invoke(`get-users-form-data${queryParams}`, {
        method: "GET",
    });
...
export const getUserByIdApi = async (id: number) => {
    const { data, error } = await supabase.functions.invoke(`get-users-details?id=${id}`, {
        method: "GET",
    });
    if (error) throw error;
    return data;
...
export const getCitiesListApi = async (countryId: number, stateId: number) => {
    return await getUsersFormDataApi({ country_id: countryId, state_id: stateId });
};

export const getNeighborhoodsListApi = async (countryId: number, stateId: number, cityId: number) => {
    return await getUsersFormDataApi({ country_id: countryId, state_id: stateId, city_id: cityId });
};
