...
import { FilterOption } from "../types/Users.types";
...
const useCreateUser = (id?: string | null, isEdit?: boolean) => {
  ...
  const isRUC =
    documentTypes
      .find((t) => t.id.toString() === formData.document_type_id)
      ?.name.includes("RUC") || false;
    ...
    const fetchOptions = async () => {
      try {
        ...
        if (!isEdit && data.accountTypes && data.accountTypes.length > 0) {
          const useType = data.accountTypes.find((t: any) =>
            t.name?.toUpperCase().includes("USE"),
          );
          if (useType) {
            setFormData((prev) => ({
              ...prev,
    ...
    if (isEdit && id && !optionsLoading) {
      const loadUserData = async () => {
        try {
          ...
          const accountTypesValues = accountTypesData.map(
            (at: any) => at.account_type_id || at.id,
          );
          console.log("=== Account Types ===", accountTypesValues);

          const roleIds =
            user.role_ids || user.roles?.map((r: any) => r.id) || [];
          ...
          const newFormData = {
            ...
            warehouse_id:
              ...
              profile?.warehouse_id?.toString() ||
              "",
            branches_id:
              user.branch_id?.toString() ||
              profile?.branch_id?.toString() ||
              profile?.branches_id?.toString() ||
              "",
            ...
            address_reference:
              user.address_reference || profile?.address_reference || "",
          ...
          if (newFormData.country_id) {
            try {
              const statesData = await getStatesListApi(
              ...
              if (newFormData.state_id) {
                ...
                if (newFormData.city_id) {
                  ...
                  setNeighborhoods(
                    neighborhoodsData.neighborhoods || neighborhoodsData || [],
                  );
                }
              }
            } catch (error) {
  ...
  const debouncedDocumentNumber = useDebounce(formData.document_number, 600);

  useEffect(() => {
    const searchDocument = async () => {
      const selectedDocType = documentTypes.find(
        (t) => t.id.toString() === formData.document_type_id,
      )?.name;

      if (!selectedDocType || !debouncedDocumentNumber || isEdit) return;

      const isDNI = selectedDocType.toUpperCase().includes("DNI");
      const isRUC = selectedDocType.toUpperCase().includes("RUC");
      const isPasaporte = selectedDocType.toUpperCase().includes("PASAPORTE");

      const minLength = isDNI ? 8 : isRUC ? 11 : isPasaporte ? 6 : null;

      if (minLength && debouncedDocumentNumber.length >= minLength) {
        try {
          setIsSearchingDocument(true);
          const response = await getUserDocumentApi({
            documentType: isDNI ? "DNI" : isRUC ? "RUC" : "PASAPORTE",
            documentNumber: debouncedDocumentNumber,
          });

          if (response && response.found) {
            setFormData((prev) => ({
              ...prev,
              name: isRUC ? response.razonSocial || "" : response.nombres || "",
              last_name: response.apellidoPaterno || "",
              last_name2: response.apellidoMaterno || "",
            }));
            setIsDocumentFound(true);
            toast({
              title: "Documento encontrado",
              description: `Datos cargados para ${isRUC ? "RUC" : isDNI ? "DNI" : "Pasaporte"}: ${debouncedDocumentNumber}`,
            });
          } else {
  ...
  // Clear fields when document type changes
  useEffect(() => {
    if (!isEdit && !optionsLoading) {
      setFormData((prev) => ({
        ...prev,
        document_number: "",
        name: "",
  ...
  }, [formData.document_type_id, isEdit, optionsLoading]);
    ...
    try {
      ...
      if (missingFields.length > 0) {
        ...
        toast({
          ...
          description:
            "Por favor complete todos los campos obligatorios (*) o su formato es inválido",
          variant: "destructive",
        });
        setLoading(false);
        return;
      ...
      const payload = {
        ...
        document_type_id: formData.document_type_id
          ? parseInt(formData.document_type_id)
          : null,
        ...
        neighborhood_id: formData.neighborhood_id
          ? parseInt(formData.neighborhood_id)
          : null,
      };
