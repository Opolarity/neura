...
const Sidebar = ({ isOpen: initialOpen, onCollapseChange }: SidebarProps) => {
  ...
  const SidebarHeader = () => (
    ...
    >
      ...
      >
        <div
          className={`w-9 h-9 bg-blue-600 rounded-xl flex items-center justify-center shadow-lg shadow-blue-900/20 transition-all duration-300 ${isCollapsed ? "hover:scale-110" : ""}`}
        >
  ...
  if (loading) {
    return (
      <div
        className={`fixed left-0 top-0 h-full bg-[#0f172a] text-white transition-all duration-300 z-30 flex flex-col ${isCollapsed ? "w-16" : "w-64"}`}
      >
        <SidebarHeader />
        <SkeletonItem />
      </div>
  ...
  return (
    <div
      className={`fixed left-0 top-0 h-full bg-[#0f172a] text-white transition-all duration-300 z-30 flex flex-col border-r border-white/5 overflow-x-hidden ${
    ...
    >
      ...
      <div className="mt-4 flex-1 flex flex-col overflow-hidden">
        <nav className="flex-1 overflow-y-auto overflow-x-hidden px-2 space-y-1 py-4 sidebar-scroll">
          {menuItems.map((item) => {
            ...
            const allSubItems =
              "subItems" in item
                ? item.subItems?.flatMap((group) => group.items) || []
                : [];
            const activeSubItem = allSubItems.find((si) =>
              isPathActive(si.location),
            );
              ...
              return (
                <div key={item.id} className="space-y-1">
                  <button
                    ...
                    className={`flex items-center gap-3 px-3 py-2.5 text-sm rounded-lg w-full text-left group relative overflow-hidden transition-all duration-200 ${
                      isActive
                  ...
                  >
                    ...
                    {!isCollapsed && (
                      <div className="flex items-center justify-between flex-1 truncate">
                        ...
                        <ChevronRight
                          className={`w-3.5 h-3.5 shrink-0 transition-transform duration-300 ${isActive ? "text-white" : isExpanded ? "rotate-90 text-blue-400" : "text-slate-600"}`}
                        />
                      </div>
                    )}
                        ...
                        <div key={subGroup.label} className="space-y-1 pt-1">
                          ...
                          {subGroup.items.map((subItem) => {
                            ...
                            return (
                              ...
                              >
                                {isSubActive && (
                                  <div className="absolute left-0 w-1 h-3 bg-blue-400 rounded-full" />
                                )}
                                {subItem.name}
                              </Link>
                            );
            ...
            return (
              ...
              >
                <div
                  className={`flex items-center justify-center w-5 h-5 shrink-0 ${isActive ? "text-white" : "group-hover:text-blue-400 transition-colors"}`}
                >
                  <Icon className="w-5 h-5" />
                </div>
                {!isCollapsed && (
      ...
      {isCollapsed && hoveredItem && !activeSubMenu && (
        <div
          ...
          style={{
            top: hoveredItem.rect.top + hoveredItem.rect.height / 2 - 16,
          }}
        >
          {hoveredItem.name}
        </div>
      ...
      {isCollapsed && activeSubMenu && (
        <div
          ...
          style={{
            top: Math.min(activeSubMenu.rect.top, window.innerHeight - 300),
          }}
        >
