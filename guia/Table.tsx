...
const Table = () => {
    ...
    const [stockChanges, setStockChanges] = useState<Map<string, number | null>>(new Map());
    ...
    const calculateRowTotal = (item: Product): number => {
        return warehousesResponse.reduce((sum, warehouse) => {
            const value = getStockValue(
                item,
                warehouse.id,
                item.stock_by_warehouse.find(s => s.warehouse_id === warehouse.id)?.stock
            );
            if (value !== null && value !== '') {
                                    ...
                                    return (
                                        <td key={wr.id}>
                                            <input
                                                type="number"
                                                value={currentValue}
                                                onChange={(e) => handleStockChange(item.variation_id, wr.id, e.target.value)}
                                                disabled={!isEditing}
                                            />
                                        </td>
                                    );
