...
FROM (
  ...
  FROM
    invoices i
    JOIN types t ON i.invoice_type_id = t.id AND (p_type IS NULL OR t.id = p_type)
    JOIN accounts a ON i.account_id = a.id
  WHERE
   (p_declared IS NULL OR i.declared = p_declared)
   AND
   ((p_min_mount IS NULL OR p_max_mount IS NULL) OR (p_min_mount IS NOT NULL AND p_max_mount IS NULL AND COALESCE(i.total_amount , 0)  >= p_min_mount ) OR (p_min_mount IS NULL AND p_max_mount IS NOT NULL AND COALESCE(i.total_amount , 0)<= p_max_mount) OR (p_min_mount IS NOT NULL AND p_max_mount IS NOT NULL AND COALESCE(i.total_amount, 0) BETWEEN p_min_mount AND p_max_mount))
   AND
   (p_search IS NULL OR (i.serie ILIKE '%' || p_search|| '%') OR (SELECT CAST (i.id AS TEXT) ILIKE '%' || p_search || '%'))
    ORDER BY
