-- Add p_discounts parameter to sp_create_order
...
BEGIN
  ...
  -- Insert order discounts from p_discounts array
  FOR v_discount IN SELECT * FROM jsonb_array_elements(p_discounts)
  LOOP
    INSERT INTO order_discounts (
      order_id, name, discount_amount, code
    ) VALUES (
      v_order_id,
      v_discount->>'name',
      COALESCE((v_discount->>'discount_amount')::NUMERIC, 0),
      v_discount->>'code'
    );
  END LOOP;
