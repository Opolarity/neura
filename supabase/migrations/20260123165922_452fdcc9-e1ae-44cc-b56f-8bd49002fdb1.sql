-- Actualizar sp_get_sale_products para filtrar stock por stock_type_id
...
BEGIN
    ...
    -- Si no se pasa stock_type_id, usar el tipo PRD por defecto
    IF p_stock_type_id IS NULL THEN
        SELECT t.id INTO v_stock_type_id
        FROM types t
        JOIN modules m ON t.module_id = m.id
        WHERE m.code = 'STK' AND t.code = 'PRD'
        LIMIT 1;
    ELSE
        v_stock_type_id := p_stock_type_id;
    END IF;
    ...
    with_details AS (
        SELECT 
            ...
            pag.variation_id,
            pag.sku,
            -- Get variation image first, otherwise get first product image
            COALESCE(
                (SELECT pi.image_url 
                 FROM product_variation_images pvi
                 INNER JOIN product_images pi ON pi.id = pvi.product_image_id
                 WHERE pvi.product_variation_id = pag.variation_id
                 ORDER BY pi.image_order
                 LIMIT 1),
                (SELECT pi.image_url 
                 FROM product_images pi
                 WHERE pi.product_id = pag.product_id
                 ORDER BY pi.image_order
                 LIMIT 1)
            ) AS image_url,
            -- Get stock filtered by stock_type_id
            COALESCE(
                (SELECT SUM(ps.stock)::integer
                 FROM product_stock ps
                 WHERE ps.product_variation_id = pag.variation_id
                   AND ps.stock_type_id = v_stock_type_id),
                0
            ) AS stock,
            COALESCE(
