...
BEGIN
  ...
  FROM (
    ...
    WHERE
      ...
      /* Filtro por stock */
      AND (p_minstock IS NULL OR subq.stock_total >= p_minstock)
      AND (p_maxstock IS NULL OR subq.stock_total <= p_maxstock)
  ...
  RETURN jsonb_build_object('productsdata', result);
END;
$function$;