CREATE OR REPLACE FUNCTION public.sp_create_order(p_user_id uuid, p_branch_id bigint, p_warehouse_id bigint, p_order_data jsonb, p_products jsonb, p_payments jsonb, p_initial_situation_id bigint, p_is_existing_client boolean DEFAULT false, p_change_entries jsonb DEFAULT '[]'::jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  INSERT INTO orders (
    ...
    (p_order_data->>'sale_type_id')::BIGINT,
  ...
  -- ============================================================
  -- STEP 2: Crear productos y stock movements
  -- ============================================================
  FOR v_product IN SELECT * FROM jsonb_array_elements(p_products)
  LOOP
    ...
    INSERT INTO stock_movements (
      product_variation_id, warehouse_id, quantity, movement_type_id,
      is_active, completed, created_by
    ) VALUES (
  ...
  -- ============================================================
  -- STEP 3: Crear pagos y movimientos financieros
  -- ============================================================
  ...
  INSERT INTO order_situations (
    order_id, situation_id, status_id, last_row
  ) VALUES (
    v_order_id, p_initial_situation_id, v_status_id, TRUE
  );

  -- ============================================================
  -- STEP 5: Nota automática
  -- ============================================================
  INSERT INTO notes (
    message, user_id
  ) VALUES (
    'Venta creada con situación inicial: ' || v_situation_name,
    p_user_id
  )
  RETURNING id INTO v_note_id;

  INSERT INTO order_notes (order_id, note_id) 
  VALUES (v_order_id, v_note_id);
