...
CREATE OR REPLACE FUNCTION sp_update_product(
  ...
  p_variations         JSONB,
  p_reset_variations   BOOLEAN DEFAULT FALSE,
...
BEGIN
  ...
  ELSE
    ...
    SELECT ARRAY(
      ...
      FROM jsonb_array_elements(p_variations) WITH ORDINALITY AS var(value, idx), jsonb_array_elements(var.value->'attributes') a
      GROUP BY var.idx
