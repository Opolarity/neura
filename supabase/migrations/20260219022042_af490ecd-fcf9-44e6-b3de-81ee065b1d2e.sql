...
-- Fix: select correct serie column based on invoice type code
...
BEGIN
  ...
  -- 5. Parse serie prefix and number from tax_serie
  ...
  -- 6. Find provider via invoice_series using the correct serie column based on type
  -- tipo_de_comprobante: 1 = factura (fac_serie), 2 = boleta (bol_serie), etc.
  IF v_invoice_type_code = '1' THEN
    -- Factura: match against fac_serie
    ...
    LIMIT 1;
  ELSIF v_invoice_type_code = '2' THEN
    -- Boleta: match against bol_serie
    SELECT ip.url, ip.token, ip.id
    INTO v_provider_url, v_provider_token, v_provider_id
    FROM invoice_series ise
    JOIN invoice_providers ip ON ip.id = ise.invoice_provider_id
    WHERE ise.bol_serie = v_serie_prefix
    LIMIT 1;
  ELSIF v_invoice_type_code = '3' THEN
    -- Nota de crédito factura
    SELECT ip.url, ip.token, ip.id
    INTO v_provider_url, v_provider_token, v_provider_id
    FROM invoice_series ise
    JOIN invoice_providers ip ON ip.id = ise.invoice_provider_id
    WHERE ise.ncf_serie = v_serie_prefix
    LIMIT 1;
  ELSIF v_invoice_type_code = '4' THEN
    -- Nota de crédito boleta
    SELECT ip.url, ip.token, ip.id
    INTO v_provider_url, v_provider_token, v_provider_id
    FROM invoice_series ise
    JOIN invoice_providers ip ON ip.id = ise.invoice_provider_id
    WHERE ise.ncb_serie = v_serie_prefix
    LIMIT 1;
  ELSIF v_invoice_type_code = '5' THEN
    -- Nota de débito factura
    SELECT ip.url, ip.token, ip.id
    INTO v_provider_url, v_provider_token, v_provider_id
    FROM invoice_series ise
    JOIN invoice_providers ip ON ip.id = ise.invoice_provider_id
    WHERE ise.ndf_serie = v_serie_prefix
    LIMIT 1;
  ELSIF v_invoice_type_code = '6' THEN
    -- Nota de débito boleta
    SELECT ip.url, ip.token, ip.id
    INTO v_provider_url, v_provider_token, v_provider_id
    FROM invoice_series ise
    JOIN invoice_providers ip ON ip.id = ise.invoice_provider_id
    WHERE ise.ndb_serie = v_serie_prefix
    LIMIT 1;
  ELSE
