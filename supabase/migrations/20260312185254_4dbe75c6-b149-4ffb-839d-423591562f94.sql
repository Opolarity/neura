...
BEGIN
  ...
  FROM types ty JOIN modules mo ON mo.id = ty.module_id and mo.code = 'STM'
  WHERE ty.code = 'ORD' LIMIT 1;

  SELECT ty.id INTO v_stock_type_id
  FROM types ty JOIN modules mo on mo.id = ty.module_id and mo.code = 'STK'
  WHERE ty.code = 'PRD' LIMIT 1;

  SELECT cl.id INTO v_movement_class_id
  FROM classes cl JOIN modules mo on mo.id = cl.module_id and mo.code = 'MOV'
  WHERE cl.code = 'ORD' LIMIT 1;
  
  SELECT ty.id INTO v_sale_movement_type_id
  FROM types ty JOIN modules mo on mo.id = ty.module_id and mo.code = 'MOV'
  ...
  CASE v_situation_code
    WHEN 'PHY' THEN v_stock_is_active := TRUE; v_stock_completed := TRUE;
    WHEN 'HDN' THEN v_stock_is_active := FALSE; v_stock_completed := FALSE;
    WHEN 'VIR' THEN v_stock_is_active := TRUE; v_stock_completed := FALSE;
    ELSE v_stock_is_active := TRUE; v_stock_completed := TRUE;
  END CASE;

  IF NOT p_is_existing_client THEN
    INSERT INTO accounts (document_type_id, document_number, name, last_name, last_name2, is_active, show)
    VALUES (
  ...
  IF p_order_data->>'notes' IS NOT NULL AND p_order_data->>'notes' <> '' THEN
    INSERT INTO notes (message, user_id) VALUES (p_order_data->>'notes', p_user_id)
  ...
  LOOP
    ...
    INSERT INTO stock_movements (
      ...
      movement_type, is_active, completed
    ) VALUES (
    ...
    IF v_existing_stock_id IS NOT NULL THEN
      UPDATE product_stock SET stock = stock - (v_product->>'quantity')::BIGINT
