...
CREATE OR REPLACE FUNCTION public.sp_open_pos_session(
  ...
  p_notes text,
  p_sale_type_id integer DEFAULT NULL
)
RETURNS json
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  INSERT INTO public.pos_sessions (
    ...
    business_account, status_id, notes, opened_at, opening_difference, sale_type_id
  )
  VALUES (
  ...
  RETURN json_build_object(
    ...
    'opened_at', NOW(),
    'sale_type_id', COALESCE(p_sale_type_id, 3)
  );
END;
$$;
