...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Helper to update stock record manually
    const updateStockRecord = async (v_id: any, w_id: any, s_id: any, qty: any) => {
      const { data: existingStock, error: fetchError } = await supabase
        .from('product_stock')
        .select('*')
        .eq('product_variation_id', v_id)
        .eq('warehouse_id', w_id)
        .eq('stock_type_id', s_id)
        .maybeSingle()

      if (fetchError) throw fetchError

      if (existingStock) {
        const newStockTotal = Number(existingStock.stock) + Number(qty)
    ...
    const { error: outMoveError } = await supabase
    ...
    // Manual Stock Update for Source
    await updateStockRecord(product_variation_id, warehouse_id, origin_stock_type_id, -Math.abs(quantity))

    // IN Movement (Destination) - Positive Quantity
    const { error: inMoveError } = await supabase
      .from('stock_movements')
      .insert([{
        product_variation_id,
    ...
    // Manual Stock Update for Destination
    await updateStockRecord(product_variation_id, warehouse_id, destination_stock_type_id, Math.abs(quantity))
