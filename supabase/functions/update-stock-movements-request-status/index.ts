...
Deno.serve(async (req) => {
    ...
    try {
        ...
        const updateStockRecord = async (mov: any) => {
            const { product_variation_id, warehouse_id, stock_type_id, quantity } = mov
        ...
        if (situation_code === 'ENV') {
            const { data: links } = await supabase
                .from('linked_stock_movement_requests')
                .select('stock_movement_id, stock_movements!inner(*)')
                .eq('stock_movement_request_id', requestId)
                .eq('approved', true)
            ...
            if (outIds.length > 0) {
                ...
                for (const l of links as any[]) {
                    if (l.stock_movements.quantity < 0) {
                        await updateStockRecord(l.stock_movements)
                    }
                }
            }
        }
