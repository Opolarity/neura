...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const exclude_product_id = body?.exclude_product_id; // excluir el producto actual
    ...
    // Llamar al SP
    const { data, error } = await supabaseClient.rpc('sp_ec_get_similar_products', {
      p_category_id:          Number(category_id),
      p_exclude_product_id:   exclude_product_id ? Number(exclude_product_id) : null,
