...
serve(async (req) => {
  ...
  try {
    ...
    const [
      ...
      supabase.from('term_groups').select('id, name, description, is_active').eq('is_active', true).order('name'),
      supabase.from('terms').select('id, name, term_group_id, is_active').eq('is_active', true).order('name'),
