...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    const { data: profile } = await supabase.from('profiles').select('warehouse_id').eq('UID', user.id).single();
    const warehouseId = profile?.warehouse_id || 1;
    // Get the situation and its status code
    const { data: situation, error: situationError } = await supabase.from('situations').select('id, status_id, statuses(code)').eq('id', situationId).single();
    ...
    const { data: previousSituation } = await supabase.from('order_situations').select('situation_id, situations(statuses(code))').eq('order_id', orderId).eq('last_row', true).maybeSingle();
    ...
    if (previousSituation) {
      await supabase.from('order_situations').update({
        last_row: false
      }).eq('order_id', orderId).eq('last_row', true);
    }
    // Insert new situation record
    const { error: insertError } = await supabase.from('order_situations').insert({
      order_id: orderId,
    ...
    // Get order products
    const { data: orderProducts, error: productsError } = await supabase.from('order_products').select('id, product_variation_id, quantity, warehouses_id, reservation').eq('order_id', orderId);
    if (productsError) {
    ...
    if (statusCode === 'RES') {
      ...
      for (const product of orderProducts){
        ...
        const { data: currentStock, error: fetchError } = await supabase.from('product_stock').select('stock').eq('product_variation_id', product.product_variation_id).eq('warehouse_id', product.warehouses_id).single();
        if (fetchError) {
          console.error('Error fetching current stock:', fetchError);
          throw fetchError;
        }
        const newStock = (currentStock?.stock || 0) - product.quantity;
        const { error: stockError } = await supabase.from('product_stock').update({
          stock: newStock
        }).eq('product_variation_id', product.product_variation_id).eq('warehouse_id', product.warehouses_id);
        if (stockError) {
      ...
      // Get order details
      const { data: order } = await supabase.from('orders').select('total, customer_name, customer_lastname, date').eq('id', orderId).single();
      // Get movement type for income
      // Get movement type for income (support 'INC' and '+')
      let movementType = null;
      {
        const { data } = await supabase.from('movement_types').select('id').eq('code', 'INC').maybeSingle();
        movementType = data ?? null;
      }
      if (!movementType) {
        const { data } = await supabase.from('movement_types').select('id').eq('code', '+').maybeSingle();
        movementType = data ?? null;
      }
      // Get payment method from order payment
      const { data: orderPayment } = await supabase.from('order_payment').select('payment_method_id').eq('order_id', orderId).maybeSingle();
      ...
      if (paymentMethodId) {
        const { data: paymentMethod } = await supabase.from('payment_methods').select('business_account_id').eq('id', paymentMethodId).maybeSingle();
